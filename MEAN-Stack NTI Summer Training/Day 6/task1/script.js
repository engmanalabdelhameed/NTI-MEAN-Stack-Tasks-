...
function renderTable(list) {
    ...
    list.forEach(function (p) {
      ...
      <td>${p.quantity === 0 ? '<span class="out-of-stock">0 Out</span>' : p.quantity}</td>
      <td class="text-center">
        <button class="action-btn edit-btn" onclick="handleEdit(${p.id})" title="Edit"><i class="fa-regular fa-pen-to-square"></i></button>
        <button class="action-btn delete-btn" onclick="handleDelete(${p.id})" title="Delete"><i class="fa-regular fa-trash-can"></i></button>
      </td>
    `;
...
document.getElementById("search-input").addEventListener("input", function (e) {
    var filtered = filterProducts(e.target.value);
...
function handleEdit(id) {
    ...
    var newName = prompt("Enter new name:", prod.name);
    var newCat = prompt("Enter new category:", prod.category);
    var newPrice = prompt("Enter new price:", prod.price);
    var newQty = prompt("Enter new quantity:", prod.quantity);

    updateProduct(id, newName, newPrice, newCat, newQty);
    renderTable(products);
}
